...
// پنل مدیریت منو — هر تغییری این‌جا فوراً روی منوی مشتری اعمال می‌شود
export default function AdminPage() {
  ...
  // پیام موفقیت که بعد از ۳ ثانیه محو می‌شود
  ...
  // اجرای یک عملیات + بارگذاری مجدد + پیام موفقیت
  ...
  const toggleCategory = (cat: Category) =>
    run(
      async () => {
        await api.patch(`/api/admin/categories/${cat.id}`, {
  ...
  const deleteCategory = (cat: Category) => {
    ...
    run(async () => {
      await api.delete<DeleteResult>(`/api/admin/categories/${cat.id}`);
    }, "دسته حذف شد");
  };
  ...
  const toggleProduct = (product: Product) =>
    run(
      ...
      product.is_available
        ? `«${product.name}» ناموجود شد و از منوی مشتری حذف شد`
        : `«${product.name}» دوباره موجود شد`,
    );
  ...
  const deleteProduct = (product: Product) => {
    ...
    setError(null);
    api
      .delete<DeleteResult>(`/api/admin/products/${product.id}`)
      .then(async (res) => {
        await loadAll();
        flash(res.detail);
      })
  ...
  const savePrice = (product: Product) => {
    ...
    }, `قیمت «${product.name}» به ${formatToman(parsed)} تغییر کرد — همین الان در منوی مشتری اعمال شد`);
  };
  ...
  return (
    <div className="mx-auto min-h-screen max-w-4xl px-4 py-6">
          ...
          {categories.map((cat) => (
            ...
            >
              <div className="flex items-center gap-3">
                <span className={`font-bold ${cat.is_active ? "" : "text-gray-400 line-through"}`}>
                  {cat.name}
                </span>
                {!cat.is_active && (
              ...
              <div className="flex items-center gap-2 text-sm">
                <button
                  ...
                  className={`rounded-lg px-3 py-1.5 text-xs font-bold ${
                    cat.is_active
                      ...
                      : "bg-pistachio-light text-pistachio"
                  }`}
                >
                ...
                <button
                  ...
                  className="rounded-lg bg-saffron-light px-3 py-1.5 text-xs font-bold text-saffron-dark"
                >
                  ویرایش
                </button>
                <button
                  ...
                  className="rounded-lg bg-berry-light px-3 py-1.5 text-xs font-bold text-berry"
                >
                  حذف
                </button>
              </div>
          ...
          {visibleProducts.map((product) => (
            ...
            >
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span
                    className={`font-bold ${product.is_available ? "" : "text-gray-400 line-through"}`}
                  >
                    {product.name}
                  </span>
              ...
              <div className="flex items-center gap-2">
                {/* ویرایش سریع قیمت: کلیک → ورودی → Enter یا کلیک بیرون */}
                {priceEditId === product.id ? (
                  ...
                  <button
                    ...
                    title="برای ویرایش قیمت کلیک کن"
                ...
                <button
                  type="button"
                  disabled={busy}
                  onClick={() => setProdModal({ mode: "edit", product })}
                  className="rounded-lg bg-saffron-light px-3 py-1.5 text-xs font-bold text-saffron-dark"
                >
                  ویرایش
                </button>
        ...
        <CategoryForm
          ...
          onSubmit={(name, displayOrder) => {
            ...
            run(async () => {
              if (isEdit) {
                await api.patch(
                  `/api/admin/categories/${catModal.category.id}`,
                  { name, display_order: displayOrder },
                );
              } else {
            ...
            }, isEdit ? "دسته ویرایش شد" : "دسته جدید ساخته شد").then(() =>
              setCatModal(null),
            );
          }}
        />
      ...
      {prodModal && (
        <ProductForm
          initial={prodModal.mode === "edit" ? prodModal.product : null}
          ...
          onSubmit={(data) => {
            ...
            run(async () => {
              if (isEdit) {
                await api.patch(
                  `/api/admin/products/${prodModal.product.id}`,
                  data,
                );
              } else {
                await api.post("/api/admin/products", data);
              }
            }, isEdit ? "محصول ویرایش شد — تغییرات فوراً در منو اعمال شد" : "محصول جدید به منو اضافه شد").then(
              () => setProdModal(null),
            );
          }}
        />
...
function ProductForm({
  ...
  const submit = (e: FormEvent) => {
    ...
    if (!name.trim()) {
      setFormError("نام محصول را وارد کنید");
      return;
    }
    if (!Number.isInteger(parsedPrice) || parsedPrice <= 0) {
      setFormError("قیمت باید عدد صحیح مثبت (به تومان) باشد");
      return;
    }
    if (!categoryId) {
      setFormError("اول یک دسته بسازید");
      return;
    }
    onSubmit({
  ...
  return (
    <Modal title={initial ? "ویرایش محصول" : "محصول جدید"} onClose={onClose}>
      <form onSubmit={submit} className="space-y-3">
        ...
        {formError && (
          <p className="rounded-xl bg-berry-light p-3 text-sm text-berry">
            {formError}
          </p>
        )}
